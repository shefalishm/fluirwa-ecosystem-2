      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: "1400px", margin: "0 auto", padding: "20px" }}>
        ...
        <div style={{ textAlign: "center", padding: "40px 20px 30px", marginBottom: "20px" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
            ...
            <span style={{ fontSize: "14px", letterSpacing: "4px", textTransform: "uppercase", color: "#4FA8D8", fontWeight: 600 }}>FluidRWA</span>
          </div>
          <h1 style={{ fontSize: "clamp(28px, 5vw, 52px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 12px", background: "linear-gradient(135deg, #ffffff 0%, #4FA8D8 50%, #7B68EE 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            ...
            <div style={{ display: "flex", borderRadius: "12px", border: "1px solid #ffffff15", overflow: "hidden" }}>
              {["grid", "table"].map(m => (
                <button key={m} onClick={() => setViewMode(m)}
                  style={{ padding: "10px 16px", background: viewMode === m ? "#4FA8D820" : "transparent", border: "none", color: viewMode === m ? "#4FA8D8" : "#667788", cursor: "pointer", fontSize: "12px", fontWeight: 600, textTransform: "uppercase", fontFamily: "inherit" }}>
                  {m === "grid" ? "◫ Cards" : "☰ Table"}
                </button>
              ))}
            </div>
        ...
        <div style={{ padding: "0 8px 12px", color: "#667788", fontSize: "13px", fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          {searchQuery && <span> for "<span style={{ color: "#4FA8D8" }}>{searchQuery}</span>"</span>}
        </div>

        {/* Vendor modal */}
        {selectedVendor && (
          <div onClick={() => setSelectedVendor(null)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "60px", padding: "60px 20px 20px", backdropFilter: "blur(8px)", overflowY: "auto" }}>
            <div onClick={e => e.stopPropagation()} ref={el => { if (el) { window.scrollTo({ top: 0, behavior: "smooth" }); el.scrollIntoView({ behavior: "smooth", block: "center" }); }}} style={{ background: "linear-gradient(145deg, #111d2e, #0c1525)", border: "1px solid #ffffff15", borderRadius: "20px", padding: "32px", maxWidth: "560px", width: "100%", maxHeight: "80vh", overflow: "auto", boxShadow: `0 20px 60px rgba(0,0,0,0.5), 0 0 40px ${(selectedVendor.categoryColor || "#4FA8D8")}10`, margin: "auto 0" }}>
              ...
              <a href={selectedVendor.url} target="_blank" rel="noopener noreferrer"
                style={{ display: "block", textAlign: "center", padding: "14px", borderRadius: "12px", background: "linear-gradient(135deg, #4FA8D8, #7B68EE)", color: "#0b1526", fontWeight: 700, fontSize: "14px", textDecoration: "none", transition: "transform 0.2s" }}
        ...
        {viewMode === "grid" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "12px", padding: "0 0 40px" }}>
            {filteredVendors.map((v, i) => {
              const color = v.categoryColor || (activeCatObj ? activeCatObj.color : "#4FA8D8");
              return (
                <div key={i} onClick={() => setSelectedVendor(v)}
                  style={{
                    background: "linear-gradient(145deg, #111d2e, #0c1525)",
                ...
                {filteredVendors.map((v, i) => {
                  ...
                  return (
                    <tr key={i} onClick={() => setSelectedVendor(v)}
                      ...
                      <td style={{ padding: "10px 16px" }}><a href={v.url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ color: "#4FA8D8", textDecoration: "none", fontSize: "12px" }}>↗</a></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div style={{ textAlign: "center", padding: "40px 20px", borderTop: "1px solid #ffffff0a", marginTop: "20px" }}>
          <p style={{ color: "#8899aa", fontSize: "14px", margin: "0 0 12px" }}>
            ...
            <a href="mailto:contact@fluidrwa.com" style={{ color: "#4FA8D8", textDecoration: "none", fontWeight: 600 }}>contact@fluidrwa.com</a>
          </p>
